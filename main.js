...
let waktu = 30;
const soundtrack = new Audio('./asset/music/soundtrack.mp3');
  ...
  tikus.forEach((x) => {
    x.addEventListener('click', (e) => {
      if (!e.target.classList.contains('kena')) {
        ...
        new Audio('./asset/music/hitt.mp3').play(); // Suara hit
...
function play() {
  const timer = setInterval(() => {
    ...
    if (waktu <= 0) {
      ...
      new Audio('./asset/music/negative_beeps-6008.mp3').play(); // Suara game over
    }
  }, 1000);
}
